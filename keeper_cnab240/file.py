...
class File:
    ...
    def process_payments(self):
        for payment in self.payments:
            register_number = 1
            payment_segments = self.bank.get_payment_segment(
                payment.get_attribute("type")
            )
            for _segment in payment_segments:
                ...
                payment_type = (
                    payment_type
                    if not issubclass(type(payment_type), Enum)
                    else payment_type.name
                )
            ...
            if line_index == 0:
                ...
                if line[8] == "C":
                    ...
                    if not payment_segment_data:
                        raise Exception(
                            "Cannot identify payment segment: "
                            + payment_header_line
                            + ", "
                            + line
                        )
